...
public class ZZMS2ExtraSerial321 extends Megasquirt {
    ...
    static String signature = "MS2Extra Serial321 \0";
    byte[] ochGetCommand = new byte[] { 'A' };
    int ochBlockSize = 169;
    static final Set<String> sigs = new HashSet<String>(
            Arrays.asList(new String[] { signature }));
    // Flags
    ...
    public void calculate(byte[] ochBuffer) {
        ...
        // TODO problem in normalizer with double and int
        boostvac = (int) ((map < barometer) ? -vacuum : boostpsig);
        if (NARROW_BAND_EGO) {
            egoVoltage = (egoV);
        } else if (LAMBDA) {
            egoVoltage = (lambda1);
    ...
    @Override
    public int getBlockReadTimeout() {
        // Increased from 100 to provide some breathing room.
        return 200;
    }
    ...
    public void loadConstants() throws IOException {
        ...
        pageBuffer = loadPage(1, 0, 1024, null, new byte[] { 114, 0, 4, 0, 0,
                4, 0 });
        ...
        pageBuffer = loadPage(2, 1024, 1024, null, new byte[] { 114, 0, 5, 0,
                0, 4, 0 });
        if (CELSIUS) {
            if (EXPANDED_CLT_TEMP) {
            } else {
        ...
        pwmidle_decelload_threshold = (double) ((MSUtils
                .getWord(pageBuffer, 46) + 0.0) * 0.1);
        ...
        pwmidle_rpmdot_disablepid = (double) ((MSUtils.getSignedWord(
                pageBuffer, 60) + 0.0) * 10.0);
        ...
        pageBuffer = loadPage(3, 2048, 1024, null, new byte[] { 114, 0, 10, 0,
                0, 4, 0 });
        ...
        staged_secondary_enrichment = (double) ((MSUtils.getWord(pageBuffer,
                741) + 0.0) * 0.0010);
        ...
        pageBuffer = loadPage(4, 3072, 1024, null, new byte[] { 114, 0, 8, 0,
                0, 4, 0 });
        if (INI_VERSION_2) {
        ...
        if (CELSIUS) {
            if (EXPANDED_CLT_TEMP) {
                idleadvance_clt = (double) ((MSUtils.getSignedWord(pageBuffer,
                        394) + -320.0) * 0.05555);
            } else {
                idleadvance_clt = (double) ((MSUtils.getSignedWord(pageBuffer,
                        394) + -320.0) * 0.05555);
